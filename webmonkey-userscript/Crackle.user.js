...
// @run-at       document-end
// @homepage     https://github.com/warren-bank/crx-Crackle/tree/webmonkey-userscript/es6
// @supportURL   https://github.com/warren-bank/crx-Crackle/issues
// @downloadURL  https://github.com/warren-bank/crx-Crackle/raw/webmonkey-userscript/es6/webmonkey-userscript/Crackle.user.js
// @updateURL    https://github.com/warren-bank/crx-Crackle/raw/webmonkey-userscript/es6/webmonkey-userscript/Crackle.user.js
...
const process_hls_url = (hls_url, vtt_url, referer_url) => {
...
// make GET request, parse JSON response, resolve Promise with data
const download_json = async (url, headers) => {
...
debug(9, async () => {
  const json_data = await download_json('https://httpbin.org/get', {"x-foo": "bar"})
...
const process_series = (series_id) => {
  const download_series_json = async (media_playlist_url) => {
    const headers = {
      "Accept": "application/json"
    }

    const json_data = await download_json(media_playlist_url, headers)
    return json_data
  }
  ...
  const request_playlist_detail = async (series_id) => {
    ...
    const media  = await download_series_json(media_playlist_url)
    const videos = []
...
const process_video = (video_id) => {
  ...
  const get_hmac = (media_detail_url, timestamp) => {
    ...
    const mac = new unsafeWindow.Digest.HMAC_SHA1()
    mac.setKey(key)
    mac.update(msg)
  ...
  const download_video_json = async (media_detail_url, timestamp, hmac) => {
    const headers = {
      "Accept"        : "application/json",
      "Authorization" : [hmac, timestamp, '117', '1'].join('|')
    }

    const json_data = await download_json(media_detail_url, headers)
    return json_data
  }

  const preprocess_video_url = (video_url) => {
  ...
  const request_media_detail = async (video_id) => {
    ...
    const get_timestamp = (d) => `${d.getUTCFullYear()}${pad_zeros(d.getUTCMonth() + 1, 2)}${pad_zeros(d.getUTCDate(), 2)}${pad_zeros(d.getUTCHours(), 2)}${pad_zeros(d.getUTCMinutes(), 2)}`
    ...
    for (const country of countries) {
      ...
      const timestamp        = get_timestamp(new Date())
      const hmac             = get_hmac(media_detail_url, timestamp)
      const media            = await download_video_json(media_detail_url, timestamp, hmac)

      if (media.MediaURLs && Array.isArray(media.MediaURLs) && media.MediaURLs.length) {
        const HLS_urls = [
          ...media.MediaURLs.filter(obj => ((obj.Type === 'AppleTV.m3u8') && !obj.UseDRM && obj.Path   )).map(obj => obj.Path   ).map(url => (url[0] === '/') ? `https:${url}` : url),
          ...media.MediaURLs.filter(obj => ((obj.Type === 'AppleTV.m3u8') &&  obj.UseDRM && obj.DRMPath)).map(obj => obj.DRMPath).map(url => (url[0] === '/') ? `https:${url}` : url)
        ]
        ...
        if (HLS_urls.length) {
          ...
          if (media.ClosedCaptionFiles && Array.isArray(media.ClosedCaptionFiles) && media.ClosedCaptionFiles.length) {
            const VTT_urls = [
              ...media.ClosedCaptionFiles.filter(obj => ((obj.Type === 'VTT') && obj.Path)).map(obj => obj.Path).map(url => (url[0] === '/') ? `https:${url}` : url),
              ...media.ClosedCaptionFiles.filter(obj => ((obj.Type === 'SRT') && obj.Path)).map(obj => obj.Path).map(url => (url[0] === '/') ? `https:${url}` : url)
            ]
...
const process_index = () => {
  const update_links = () => {
    ...
    const labels = [...unsafeWindow.document.querySelectorAll('.mediaItem > .Item > .mediaStill + .mediaBox:not([x-modified])')]
...
const init = () => {
  ...
  if (unsafeWindow.history) {
    ...
    unsafeWindow.history.replaceState = (state, title, url) => {
      process_site_url(url)
